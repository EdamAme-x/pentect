...
static CONTEXT: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?i)(?:identification|identity|id[-‑ ]?number|id[-‑]?nummer|nummer|verified|verification|profile|eligibility|compliance|copy of|provide a copy|copie|submit|certification numbers?|reference number|referenznummer|sensitive data|občutljivimi podatki|official registration|anmeldung|ausweisen|secure portal|employee portal|receipt|receipts|expense|expenses|chi tiêu|biên lai|secret code|passport|passaporte|pasportu|passeport|passaporto|reisepass|paszport|contribuinte|tax[-_ ]?(?:id|number)|fiscal|fiscale|identification fiscale|данъчен номер|credit card|card number|num[eé]ro de carte|carte de cr[eé]dit|kreditnou kartou|číslo karty|bank account|account number|conto bancario|licencia de conducir|n[uú]mero de identificaci[oó]n|ταυτοτητας|ταυτότητας|κοινωνικής ασφάλισης|διπλώματος οδήγησης)",
    )
    .unwrap()
});

static CANDIDATE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\b(?:[A-Z0-9][A-Z0-9-]{6,38}[A-Z0-9]|[0-9][0-9 .-]{6,28}[0-9])\b").unwrap()
});
...
impl Detector for ContextualIdDetector {
    fn detect(&self, view: &NormalizedView) -> Vec<Span> {
        ...
        for m in CANDIDATE.find_iter(s) {
            if plausible_id(m.as_str()) && has_near_context(s, m.start(), m.end()) {
                out.push(Span {
