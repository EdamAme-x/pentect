...
mod tests {
    ...
    #[test]
    fn completed_tool_inputs_restore_known_values_without_shell_rewriting() {
        let handle = "<<KEYED_SECRET_a2c25e122d2e002f>>";
        let secret = "quoted \"; Remove-Item x\nnext ' $()";
        let mut resolve = |text: &str| Ok(text.replace(handle, secret));
        let restored = resolve_tool_input_json(
            &serde_json::json!({
                "command": format!("echo {handle}"),
                "path": format!("fixtures/{handle}.txt"),
                "content": format!("password={handle}"),
                "mcp": {"authorization": handle},
            })
            .to_string(),
            Some("Bash"),
            &mut resolve,
        ...
        assert_eq!(restored["command"], format!("echo {secret}"));
        assert_eq!(restored["path"], format!("fixtures/{secret}.txt"));
        assert_eq!(restored["content"], format!("password={secret}"));
        assert_eq!(restored["mcp"]["authorization"], secret);
        assert!(!restored.to_string().contains("pentect exec"));
        assert!(!restored.to_string().contains("script-b64"));
    }
