...
## Optional privacy groups (since 0.0.92)

Personal information such as email addresses and phone numbers is **off by
default**. UUIDs, internal hosts, and IP addresses are also **off by default**.
Enable either group in your Pentect configuration:
...
These options apply to text and recognized image content. Credentials, credit
cards, and explicit `mask(...)` markers remain protected with both options off.
A password that happens to look like an email or UUID is still a credential.
See [configuration](/reference/configuration/#optional-personal-and-internal-information)
for precedence and restart requirements. Upgrading does not retain the old
PII/internal defaults: set the options above to keep those groups enabled.
...
| Session ID or session token | Built in for sensitive structured keys, supported key/value text, and structurally valid token formats such as JWT | UUID shape alone is not proof of a credential. Non-secret identifier masking requires `protection.internal = true`; credential-bearing cookie and session contexts remain protected. |
...
| `CliCredentialDetector` | Secret | Sensitive long options, contextual login password options, sensitive `configure set` values, declared positional credential commands, and plaintext password arguments in supported PowerShell shapes | Shell-token boundaries, metadata/non-value options, command-shape, and mask/restore regression fixtures; ambiguous short or positional arguments require verified command context |
